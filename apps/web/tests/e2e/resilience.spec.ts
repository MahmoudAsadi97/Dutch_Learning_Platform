...
test.describe("resilience", () => {
  test("the connection banner appears when calls fail and clears after a successful retry", async ({ page }) => {
    ...
    await page.getByRole("navigation", { name: "Hoofdmenu" }).getByRole("link", { name: "Missie" }).click();
  ...
  test("the home page shows the budget counters and the empty pricing table", async ({ page }) => {
    await page.goto("/");
