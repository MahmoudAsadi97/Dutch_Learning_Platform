...
export function CurriculumAudio({ endpoint, parts = 1, transcript, disabled = false }: { endpoint: string; parts?: number; transcript?: LearningCopy; disabled?: boolean }) {
  ...
  async function play() {
    ...
    try {
      if (!urlRef.current) {
        const response = await apiFetch(`${endpoint}?part=${part}`, { method: "POST", signal: controllerRef.current.signal });
      ...
      if (player) {
        ...
        await player.play().catch(() => setError("Gebruik de afspeelknop hieronder om te luisteren."));
      }
    } catch (cause) {
      if (!(cause instanceof DOMException && cause.name === "AbortError")) setError(friendlyError(cause instanceof ApiError ? cause.status : undefined));
    } finally { setBusy(false); }
  }
  return <div className="curriculum-audio"><div className="audio-intro"><span className="audio-icon"><Icon name="headphones" size={27}/></span><div><strong>Luister aandachtig</strong><p>Je mag opnieuw luisteren. Je hoort een synthetische Belgische stem.</p></div></div><div className="audio-actions">{parts > 1 && <label>Fragment <select value={part} disabled={disabled || busy} onChange={event => { clearAudio(); setPart(Number(event.target.value)); }}>{Array.from({ length: parts }, (_, index) => <option value={index} key={index}>{index + 1} / {parts}</option>)}</select></label>}<button className="button" onClick={() => void play()} disabled={disabled || busy}><Icon name="volume"/>{busy ? "Even laden…" : "Luister naar het fragment"}</button></div><audio ref={audioRef} controls hidden={!url} aria-label="Luisterfragment"/>{error && <p role="status">{error}</p>}{transcript && <details className="transcript-help"><summary>Lees mee als je hulp nodig hebt</summary><p className="story-copy"><LearningText text={transcript}/></p></details>}</div>;
}
...
export function CurriculumRecorder({ onReady, onActivityChange, disabled = false, minWords = 1, maxWords = 150, maxSeconds = 60, initialTranscript = "" }: {
  ...
  useEffect(() => () => { if (urlRef.current) URL.revokeObjectURL(urlRef.current); onActivityRef.current?.(false); }, []);
  async function upload(recording: Recording) {
    ...
    try {
      const response = await apiJson<{ audio: { asset_id: string }; transcript: { text: string } }>("speech/transcribe", { method: "POST", formData: form });
    ...
    } catch (cause) { setError(cause instanceof ApiError ? friendlyError(cause.status) : "We konden je niet goed verstaan. Probeer een nieuwe opname in een rustige ruimte."); }
    finally { setBusy(false); }
  }
