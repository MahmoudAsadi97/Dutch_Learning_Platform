...
export function CurriculumLesson({ stageId }: { stageId: string }) {
  ...
  function changeSection(next: Section) {
    ...
    setSection(next); setFeedback(null); setError("");
  }
  ...
  return <div className="curriculum-lesson">
    ...
    <div className="learning-workspace"><aside className="unit-sidebar"><p className="eyebrow">STAP VOOR STAP</p><nav aria-label="Onderdelen van dit niveau">{sections.map(item => <button key={item.id} disabled={busy || recordingBusy} aria-current={section === item.id ? "step" : undefined} onClick={() => changeSection(item.id)}><Icon name={item.icon}/><span>{item.label}</span>{completed.includes(item.id as Skill) && <Icon name="check" size={15}/>}</button>)}</nav><div className="unit-test-card"><strong>{completed.length} van 4 vaardigheden geoefend</strong><p>Laat daarna zelfstandig zien wat je kunt.</p>{progress.test_available || stage.admin_bypass ? <Link className="button" href={`/learn/${stageId}/test`}>Naar de eindtoets<Icon name="arrow" size={16}/></Link> : <span className="test-unavailable"><Icon name="shield" size={16}/>Eindtoets opent na vier vaardigheden</span>}</div></aside>
      <div className="unit-content" key={section}><div className="unit-section-title"><p className="eyebrow">{stageLabel(stageId)} · {sections.find(item => item.id === section)?.label}</p><h2>{section === "words" ? "Woorden die je kunt gebruiken" : section === "grammar" ? "Zo bouw je een zin" : section === "reading" ? "Een verhaal om te ontdekken" : section === "listening" ? "Luister naar de situatie" : section === "speaking" ? "Breng het gesprek op gang" : "Schrijf je eigen boodschap"}</h2></div>
        ...
        {feedback && <div className={`practice-feedback ${feedback.completed ? "complete" : "retry"}`} role="status"><h3>{feedback.completed ? "Een stap verder." : "Probeer het nog eens."}</h3><LearningText text={feedback.feedback}/>{feedback.total !== undefined && <p>{feedback.correct} van {feedback.total} juist</p>}{feedback.completed && section !== "writing" && <button className="button secondary" disabled={busy || recordingBusy} onClick={() => changeSection(sections[sections.findIndex(item => item.id === section) + 1].id)}>Volgende vaardigheid<Icon name="arrow" size={17}/></button>}</div>}
