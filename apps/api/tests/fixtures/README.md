...
- `dutch_sentence.wav` — reserved name for a real recorded Dutch sentence captured on the owner's laptop through the microphone check page and exported with `python -m dlp.cli export-recording <request id>` (see `OWNER_ACTIONS.md`). Absent until the owner adds it.
