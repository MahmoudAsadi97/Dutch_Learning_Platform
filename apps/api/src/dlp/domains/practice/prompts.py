...
def propose_action_messages(scenario: Scenario, state: AppointmentState, history: list[tuple[str, str]],
    ...
    system = (
        ...
        "- propose_slot: de leerder stelt zelf een moment voor (slot_id als het overeenkomt met een beschikbaar moment)\n"
        "- accept_slot: de leerder aanvaardt een aangeboden moment (slot_id verplicht)\n"
...
def character_reply_messages(scenario: Scenario, state: AppointmentState, history: list[tuple[str, str]],
    ...
    system = (
        ...
        "Regels: bevestig nooit een afspraak die de toepassing niet aanvaard heeft. Verzin geen andere momenten. "
        "Antwoord met één of twee korte zinnen. Geen uitleg, geen vertaling."
    )
    messages: list[tuple[str, str]] = [("system", system)]
    for role, text in history[-6:]:
        messages.append(("assistant" if role == "character" else "user", text))
