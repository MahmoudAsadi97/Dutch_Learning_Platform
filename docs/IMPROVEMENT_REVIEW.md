...
| Clearing a draft was ignored; concurrent saves could restore an older version. | Serialize saves, save empty drafts, drain pending saves before submission and lesson-step navigation, show save failures and retry. Treat the draft as pending until queued writes finish, even when text returns to its original value. | Client unit tests plus browser clear/reload, delayed save and failed navigation tests. |
...
| Home prioritised diagnostics over the learning task. | Add a mission entry panel and four real skill cards, retaining diagnostics and usage below. Keep diagnostic-table scrolling inside the card on narrow screens. | Responsive browser journey, inspected desktop/phone-width screenshots and production build. |
